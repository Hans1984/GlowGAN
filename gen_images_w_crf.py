...
def generate_images(
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        w, e, gamma, beta = gen_utils.get_w_e_gamma_beta_from_seed(G, batch_sz, device, truncation_psi, seed=seed,
                                      centroids_path=centroids_path, class_idx=class_idx)
        img = gen_utils.w_e_to_img_crf(G, w, e, gamma, beta, to_np=True)
        img_np = np.squeeze(img)
        img_np = cv2.cvtColor(img_np.astype(np.float32), cv2.COLOR_BGR2RGB)
        save_name = outdir + '/' + str(seed) + '.hdr'
        print(save_name)
