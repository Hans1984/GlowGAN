...
def training_loop(
    ...
    if rank == 0:
        ...
        e = torch.empty([batch_gpu, 1], device=device)
        gamma = torch.empty([batch_gpu, 1], device=device)
        beta = torch.empty([batch_gpu, 1], device=device)
        img = misc.print_module_summary(G, [z, c, e, gamma, beta])
    ...
    if rank == 0:
        ...
        grid_e = torch.randn([labels.shape[0], 1], device=device).split(batch_gpu)
        grid_gamma = torch.normal(mean = 0.9, std = 0.1, size = (labels.shape[0], 1), device = device).split(batch_gpu)
        grid_beta = torch.normal(mean = 0.6, std = 0.1, size = (labels.shape[0], 1), device = device).split(batch_gpu)

        images = torch.cat([G_ema(z=z, c=c, e=e, gamma = gamma, beta = beta, noise_mode='const').cpu() for z, c, e, gamma, beta in zip(grid_z, grid_c, grid_e, grid_gamma, grid_beta)]).numpy()
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            all_gen_e = torch.randn([len(phases) * batch_size, 1], device=device)
            all_gen_e = [phase_gen_e.split(batch_gpu) for phase_gen_e in all_gen_e.split(batch_size)]

            all_gen_gamma = torch.normal(mean = 0.9, std = 0.1, size = (len(phases) * batch_size, 1), device=device)
            all_gen_gamma = [phase_gen_gamma.split(batch_gpu) for phase_gen_gamma in all_gen_gamma.split(batch_size)]

            all_gen_beta = torch.normal(mean = 0.6, std = 0.1, size = (len(phases) * batch_size, 1), device=device)
            all_gen_beta = [phase_gen_beta.split(batch_gpu) for phase_gen_beta in all_gen_beta.split(batch_size)]
        ...
        for phase, phase_gen_z, phase_gen_c, phase_gen_e, phase_gen_gamma, phase_gen_beta in zip(phases, all_gen_z, all_gen_c, all_gen_e, all_gen_gamma, all_gen_beta):
            ...
            for real_img, real_c, gen_z, gen_c, gen_e, gen_gamma, gen_beta in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c, phase_gen_e, phase_gen_gamma, phase_gen_beta):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gen_e=gen_e, gen_gamma=gen_gamma, gen_beta=gen_beta, gain=phase.interval, cur_nimg=cur_nimg)
