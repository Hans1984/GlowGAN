...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', force_fp32=False, update_emas=False):
        ...
        if not self.is_torgb:
            x = filtered_lrelu.filtered_lrelu(x=x, fu=self.up_filter, fd=self.down_filter, b=self.bias.to(x.dtype),
                up=self.up_factor, down=self.down_factor, padding=self.padding, gain=gain, slope=slope, clamp=self.conv_clamp)
        else:
            # print('x before relu max', torch.max(x))
            # print('x before relu mean', torch.mean(x))
            # print('x before relu min', torch.min(x)) 
            x = self.act(x)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, e, gamma, beta, is_tmo=True, **layer_kwargs):
        ...
        if is_tmo:
           ...
           e = e[:,None, None]
           e = e.expand(-1, x_size[1], x_size[2], x_size[2]) 

           gamma = gamma[:,None, None]
           gamma = gamma.expand(-1, x_size[1], x_size[2], x_size[2]) 

           beta = beta[:,None, None]
           beta = beta.expand(-1, x_size[1], x_size[2], x_size[2]) 
           

           #x = x + 1e-4
           x = torch.pow(1.4142, e)*x
           #x = torch.pow(2.0, e)*x
           #x = x + 1e-4
           x = torch.where(x > ones, ones, x)
           #print('x before crf', x[1, 1, 1, 1])
           #print('add epsilon')
           x = (1 + beta)*(torch.pow((x+1e-7), gamma))/(torch.pow((x+1e-7), gamma) + beta)
           #x = x + 1e-4
           #print('x after crf', x[1, 1, 1, 1])
           x = 2.0*x - 1.0
           # print('x max', torch.max(x))
           #print('x min', torch.min(x))
           # print('x mean', torch.mean(x))
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, e, gamma, beta, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        img = self.synthesis(ws, e, gamma, beta, update_emas=update_emas, **synthesis_kwargs)
        # print('img max', torch.max(img))
        # print('img mean', torch.mean(img))
